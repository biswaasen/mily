...
export const WaveBars: React.FC<WaveBarsProps> = ({ status, barsRef }) => {
  return (
    <>
      ...
      `}</style>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 2.5, lineHeight: 0 }}>
        {Array.from({ length: WAVE_LINES }).map((_, i) => (
          <div
            key={i}
            ref={(el) => { barsRef.current[i] = el; }}
            className={`wave-bar ${status === 'processing' ? 'processing' : ''}`}
            style={{
              width: 2,
              height: status === 'recording' ? 10 : status === 'processing' ? 4 : 2,
              borderRadius: 999,
              background: '#fff',
              opacity: 0.8,
